...
int kReenter;
Process		procTable[NR_TASKS+NR_PROCS];
Task		taskTable[NR_TASKS] = {
			{(t_4)taskTty, STACK_SIZE_TTY, "tty"},
		};
Task		userProcTable[NR_PROCS] = {
			{(t_4)testA, STACK_SIZE_TESTA, "testA"},
                        {(t_4)testB, STACK_SIZE_TESTB, "testB"},
			{(t_4)testC, STACK_SIZE_TESTC, "testC"}
		};
void* irqTable[NR_IRQ];
void* syscallTable[NR_SYSCALL] = {
			sysGetTicks,
			sysWrite
		};
t_4		ticks;
